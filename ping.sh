...
PING_NS=clab-vlan-client1

if [ "$APP_NAME" == "clabernetes" ]; then
    PING_NS="client1"
fi

ping_test() {
    ip netns exec ${PING_NS} ping -c 1 -w 1 $1 > /dev/null
    if [ $? -eq 0 ]; then
...
if [ "$CASE" == "no-tag" ]; then
    ip netns exec ${PING_NS} ping -c 1 10.1.0.2
fi

if [ "$CASE" == "single-tag-10" ]; then
    ip netns exec ${PING_NS} ping -c 1 10.1.1.2
fi

if [ "$CASE" == "single-tag-11" ]; then
    ip netns exec ${PING_NS} ping -c 1 10.1.2.2
fi

if [ "$CASE" == "double-tag" ]; then
    ip netns exec ${PING_NS} ping -c 1 10.1.3.2
fi
